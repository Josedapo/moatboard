...
for (const ticker of TICKERS) {
  try {
    ...
    // Create position + first buy atomically via CTE. Shares default to 1
    // for these dogfood inserts — the validation is about framework routing,
    // not about cost basis realism.
    ...
    `;
    console.log(
      `  ${ticker.padEnd(6)} → #${rows[0].id} at $${Number(rows[0].price).toFixed(2)}`,
    );
  } catch (err) {
