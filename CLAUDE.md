...
**Universal ticker dispatcher** at `/dashboard/ticker/[symbol]` (server component, post-2026-04-27): redirects to the right ficha based on user's relationship with the ticker — `/dashboard/position/[id]` for in_portfolio / discarded / outside_circle / drafts with cached analysis; `/dashboard/watchlist/[ticker]` for watchlist; `/dashboard/analyze/[ticker]` for first contact. Reusable from any surface (Discovery leaderboard's "Ver →" button, future Inbox/History links) without callers needing to know which page owns the user's specific case.
